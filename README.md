...
> - 会话统计实时常驻:轮数/步数/耗时/tok·s/缓存命中/输入输出 token 常驻显示并实时刷新;长会话历史同步修复
> - 插话发送:任务运行中 Ctrl+Enter 立即打断当前回合插入新指令(与 Web 端一致),Enter 排队
> - 新建对话自动归属当前 VS Code 目录(工作区→打开文件目录→用户主目录兜底)
